...
Below we see two function definitions which perform the same task but implement different programming techniques. The first utilizes recursive function calls, while the second uses a `while`{.r} loop to repeat sample generation until the acceptance criterion is met.
...
I know the `::`{.r} operator has a performance cost. It may be interesting to try the benchmarking with loaded packages, too.
